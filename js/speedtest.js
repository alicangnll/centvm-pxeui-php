...
function Speedtest() {
  ...
  console.log("HizTesti by Federico Dossena v5.2.2");
}
...
Speedtest.prototype = {
  ...
  loadServerList: function(url,result) {
    if (this._state == 0) this._state = 1;
    if (this._state != 1) throw "You can't add a server after server selection / Seçtikten sonra server eklenemez";
    this._settings.mpot = true;
  ...
  getSelectedServer: function() {
    if (this._state < 2 || this._selectedServer == null)
      throw "No server is selected / Hiçbir server seçilmedi";
  ...
  selectServer: function(result) {
    if (this._state != 1) {
      if (this._state == 0) throw "No test points added / Hiçbir test noktası eklenmedi";
      if (this._state == 2) throw "Server already selected / Server zaten seçili";
      if (this._state >= 3)
        throw "You can't select a server while the test is running / Test esnasında server değiştiremezsiniz.";
    }
